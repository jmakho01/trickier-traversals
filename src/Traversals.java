...
public class Traversals {
  ...
  public static <T> List<T> collectLevelOrderValues(TreeNode<T> node) {
    ...
    if(node == null) { return theList; }
    theList.add(node.value);
    collectLevelOrderValues(node, theList);
    return theList;
  }

  public static <T> void collectLevelOrderValues(TreeNode<T> node, List<T> list) {
    if(node == null) { return; }
    if(node.left != null) { list.add(node.left.value); }
    if(node.right != null) { list.add(node.right.value); }
  ...
  public static int countDistinctValues(TreeNode<Integer> node) {
   if(node == null) { return 0; }
    ...
    countDistinctValues(node, distinct);
    return distinct.size();
  }

  public static void countDistinctValues(TreeNode<Integer> node, Set<Integer> theSet) {
    if(node == null) { return; }
    if(!theSet.contains(node.value)) { theSet.add(node.value); }
    countDistinctValues(node.left, theSet);
    countDistinctValues(node.right, theSet);
  }
  ...
  public static boolean hasStrictlyIncreasingPath(TreeNode<Integer> node, int theValue) {
    if(node == null) { return true; }
    if(node.value < theValue) { return false; }
    return hasStrictlyIncreasingPath(node.left, node.value) || hasStrictlyIncreasingPath(node.right, node.value);
  }
